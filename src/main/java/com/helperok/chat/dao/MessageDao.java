...
public interface MessageDao extends PagingAndSortingRepository<Message,Long>{
    ...
    @Query(value = "select m from Message m where (m.senderId=?1 and m.receiverId =?2) or (m.senderId=?2 and m.receiverId=?1) order by m.createTime desc LIMIT 1",nativeQuery = true)
